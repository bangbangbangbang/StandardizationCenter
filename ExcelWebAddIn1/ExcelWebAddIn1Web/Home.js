(function () {
    ...
    function hightlightHighestValue() {
        ...
        Excel.run(function (ctx) {
            // 创建选定范围的代理对象，并加载其属性
            var sourceRange = ctx.workbook.getSelectedRange().load("values, rowCount, columnCount, address");
           
            // 运行排队的命令，并返回承诺表示任务完成
            return ctx.sync()
                .then(function () {

                    $.alert(sourceRange.address);

                    var highestRow = 0;
                    var highestCol = 0;
                    var highestValue = sourceRange.values[0][1];
